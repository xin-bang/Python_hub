...
def get_path_and_seq(df, path):
    if '生产编号' and '病原种拉丁名' not in df.columns:
        raise ValueError("DataFrame 必须包含 '病原种拉丁名' 和 '生产编号' 列")  #由raise 引起 ValueError主动抛出异常
    ...
    for idx, row in df.iterrows():       # 遍历 DataFrame 中的每个生产编号
        ...
        # 查找包含生产编号的文件
        found_file = None
        for filename in os.listdir(path): # 遍历path下所有的文件，并以list格式展开
            if re.search(prod_id, filename) and filename.endswith('.seq'): #re.search查找list中第一个prod_id，没有则None
                found_file = filename
                break
...
def run_local_blast(sequence, db_path, output_file):
    ...
    blast_cline = NcbiblastnCommandline(
        ...
        out=output_file, 
        max_target_seqs=args.m,  
        num_threads=20
    )
 
    stdout, stderr = blast_cline() #执行定义的命令行
...
def parse_blast_results(output_file):
    ...
    def format_row(row):
        return f"{row['比对物种标题']}|count:{row['count']}({row['percentage']})|pid_cov:{row['max_pid_cov']}"  #花括号
